...
const SearchBar = ({
  ...
  const filteredPodcasts = React.useMemo(() => {
    let result = podcasts.filter(
      (podcast) =>
        podcast.title?.toLowerCase().includes(searchTerm.toLowerCase()) || ""
    );
    if (selectedGenre) {
      result = result.filter(
        (podcast) => podcast.genres?.includes(selectedGenre) || false
      );
    }
  ...
  return (
    <>
      ...
      <div style={{ padding: "1rem" }}>
        ...
        <span>
          Page {currentPage} of{" "}
          {Math.ceil(filteredPodcasts.length / itemsPerPage)}
        </span>
        <button
          onClick={() => setCurrentPage((prev) => prev + 1)}
          disabled={
            currentPage === Math.ceil(filteredPodcasts.length / itemsPerPage)
          }
          style={{
            padding: "0.5rem 1rem",
            marginLeft: "0.5rem",
